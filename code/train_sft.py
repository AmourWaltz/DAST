...
class ModelArguments:
    model_name: str = field(default="llama31", metadata={"help": "Model name.", "choices": model_path_dict.keys()})
...
@dataclass
class LoraArguments:
    lora_use: bool = field(default=False, metadata={"help": "Whether to use LoRA."})
...
def train():
    ...
    training_args.output_dir = os.path.join(training_args.output_dir.format(data_args.dataset), 
                                            "{}_{}_{}_{}{}{}".format(model_args.model_name, 
                                                              data_args.dataset, 
                                                              data_args.data_file, 
                                                              data_args.save_suffix,
                                                              "_bnb" if model_args.bnb_use else "",
                                                              "_lora" if lora_args.lora_use else ""))
